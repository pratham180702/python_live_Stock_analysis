...
if(choice1 == '1'):
    ...
    if(choice == '1'):
        ...
        plt.plot(df.index, df['High'],"g", label='High, prev. High = '+str(df['High'].iloc[-1])[0:6]+" USD")
        plt.plot(df.index, df['closing'],"black",label = 'Closing, prev. closing = '+str(df['closing'].iloc[-1])[0:6]+" USD")
        plt.plot(df.index, df['low'],"r",label = 'low, prev. low = '+str(df['low'].iloc[-1])[0:6]+" USD")
        plt.xlabel("From "+str(last_date[0:10])+" TO "+str(current_date[0:10])+ "---->")
        plt.ylabel("Price in USD")
        ...
        plt.axhline(y=df['Pivot point'].iloc[-1], color='black', linestyle='-',label = "PP = "+str(df['Pivot point'].iloc[-1])[0:6]+" USD")
        plt.axhline(y=df['Resistance 1'].iloc[-1], color='g', linestyle='--',label = "1st Resistance = "+str(df['Resistance 1'].iloc[-1])[0:6]+" USD")
        plt.axhline(y=df['Resistance 2'].iloc[-1], color='g', linestyle='--')
        plt.axhline(y=df['Resistance 3'].iloc[-1], color='g', linestyle='--')
        plt.axhline(y=df['Support 1'].iloc[-1], color='r', linestyle='--',label = "1st Support = "+str(df['Support 1'].iloc[-1])[0:6]+" USD")
        plt.axhline(y=df['Support 2'].iloc[-1], color='r', linestyle='--')
        plt.axhline(y=df['Support 3'].iloc[-1], color='r', linestyle='--')
        plt.legend(loc="upper right")
        plt.title("UPCOMING PIVOT POINT, RESISTANCES AND SUPPORTS OF "+stock)
        plt.show()
        ...
        plt.axhline(y=df['Woodie PP'].iloc[-1], color='black', linestyle='-',label = "WOODIE PP = "+str(df['Woodie PP'].iloc[-1])[0:6]+" USD")
        plt.axhline(y=df['WR1'].iloc[-1], color='g', linestyle='--',label = "Woodie R1 = "+str(df['WR1'].iloc[-1])[0:6]+" USD")
        plt.axhline(y=df['WR2'].iloc[-1], color='g', linestyle='--')
        plt.axhline(y=df['WR3'].iloc[-1], color='g', linestyle='--')
        plt.axhline(y=df['WS1'].iloc[-1], color='r', linestyle='--',label = "Woodie S1 = "+str(df['WS1'].iloc[-1])[0:6]+" USD")
...
#intraday change percent alert analysis 
    ...
    if abs(last_change) > 0.0004:
        ...
        pywhatkit.sendwhatmsg(phone, stock+" Alert: last minute closing of your stock has crossed your stop loss closing limit by the change percent of : " + str(last_change)[0:10] + "%",hour,min+1)
